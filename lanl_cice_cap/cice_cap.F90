...
! 00/00/20: S. Moorthi some optimization
module cice_cap_mod
...
!!$omp parallel do private(i,j,i1,j1)
      do j = jlo,jhi
        ...
        do i = ilo,ihi
          ...
          if(dataPtr_pbot(i1,j1) > 0.0) &
          potT   (i,j,iblk) = dataPtr_Tbot   (i1,j1) * (100000.0d0/dataPtr_pbot(i1,j1))**0.286 ! Potential temperature (K)
...
!!$omp parallel do private(i,j,i1,j1,ui,vj,sina,cosa)
      do j = jlo,jhi
        ...
        do i = ilo,ihi
          ...
          sina = sin(ANGLET(i,j,iblk))
          cosa = cos(ANGLET(i,j,iblk))
          dataPtr_strairxT(i1,j1) = ui*cosa - vj*sina  ! air ice stress
          dataPtr_strairyT(i1,j1) = ui*sina + vj*cosa  ! air ice stress
          ui   = -strocnxT(i,j,iblk)
          vj   = -strocnyT(i,j,iblk)
          dataPtr_strocnxT(i1,j1) = ui*cosa - vj*sina  ! ice ocean stress
...
#else
        field = ESMF_FieldCreate(grid, ESMF_TYPEKIND_R8, indexflag=ESMF_INDEX_DELOCAL,  &
!                                totalLWidth=(/1,1/), totalUWidth=(/1,1/),i             &
                                 ungriddedLBound=(/1/), ungriddedUBound=(/max_blocks/), &
                                 name=field_defs(i)%shortname, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, line=__LINE__, file=__FILE__)) return
#endif
